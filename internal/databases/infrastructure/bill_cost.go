...
type BillCostServiceDay struct {
	...
	ServiceSubName string  `json:"service_sub_name" gorm:"column:service_sub_name;comment:'Service custom subtype name'"` // 服务备用自定义子名称
...
func SelectBillCostServiceDay(db *gorm.DB, dateStart, dateEnd string) ([]BillCostServiceDay, error) {
	...
	res := db.Where("date_start IN ?", dates).Find(&d)
	return d, res.Error
}
...
type BillCostServiceMonthly struct {
	...
	ServiceSubName string  `json:"service_sub_name" gorm:"column:service_sub_name"` // 服务备用自定义子名称
